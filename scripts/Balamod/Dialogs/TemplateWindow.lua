...
local detailsPopup = dofile(Global_Config.SYS_EXTENSIONS_DIR("Balamod", "Dialogs", "DetailsWindow.lua"))

local function Show()
    ----------------------------------------------------------------------------------------
    -- Initial setup of the dialog and Reset method.
    ----------------------------------------------------------------------------------------
    local dlg = Dialog(Balamod_I18n:get("tmp_title"));
    local templates = {};
    local templateNames = {};
    ...
    local function SetDefaultTemplates()
        ...
        for i, v in ipairs(templates) do
            templates[i].file = Global_Config.SYS_EXTENSIONS_DIR("Balamod", "Images", v.file)
        end
    end
    ...
    local function SetFileDisplayMode(fileMode)
        pluginData.utils.set_visible(dlg, fileMode == false,
            { "templateDropdown", "resetButton", "detailsButton", "createFromPresetButton" });
